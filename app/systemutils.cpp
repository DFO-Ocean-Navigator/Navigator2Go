...
bool IsProcessRunning(const QString &processName) {
  ...
  static const auto &postfix{QStringLiteral(
  ...
  const auto args{QStringList()
                  << QStringLiteral("-c") << prefix + processName + postfix};
...
auto IsProcessRunning(const QString &processName) { return false; }
#endif
...
void SendDesktopNotification(const QString &title, const QString &message) {
  ...
  const auto &command{
      QStringLiteral("notify-send '%1' '%2' '-i' "
                     "/usr/share/pixmaps/ubuntu-logo.png '-t' 5000")
          .arg(title)
          .arg(message)};
#else
#endif
