...
/***********************************************************************************/
DialogTHREDDSLogs::DialogTHREDDSLogs(const QString &threddsLogFolder,
                                     QWidget *parent)
    : QDialog{parent}, m_ui{new Ui::DialogTHREDDSLogs}, m_logFolder{
                                                            threddsLogFolder} {
  m_ui->setupUi(this);
  ...
  QObject::connect(&m_reloadLogFileTimer, &QTimer::timeout, this, [this]() {
    this->loadLogFile(this->m_ui->comboBoxLogFiles->currentText());
  });
  m_reloadLogFileTimer.start();
}

/***********************************************************************************/
DialogTHREDDSLogs::~DialogTHREDDSLogs() { delete m_ui; }
...
/***********************************************************************************/
void DialogTHREDDSLogs::on_comboBoxLogFiles_currentIndexChanged(
    const QString &arg1) {
  loadLogFile(arg1);
}
