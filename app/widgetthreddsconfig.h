...
private slots:
  ...
  /// Browse THREDDS directory button callback: opens the THREDDS content
  /// directory in the system's file explorer.
  void on_pushButtonBrowseTHREDDSDir_clicked();

private:
  /// Ensures the THREDDS catalog path is valid. Creates correct path if
  /// non-exisent.
  ...
  const QString *const m_catalogLocation{
      nullptr}; ///< Ptr to THREDDS catalog location in user settings
};
