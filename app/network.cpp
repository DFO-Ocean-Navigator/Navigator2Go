...
/***********************************************************************************/
void MakeAPIRequest(QNetworkAccessManager &nam, const QString &APIURL,
  ...
  QObject::connect(
      reply, &QNetworkReply::finished, pcontext,
      [context = std::move(context), replyHandler = std::move(replyHandler),
       errorHandler = std::move(errorHandler), reply]() mutable {
        context.reset(); // Clear context

        // Check for errors
        if (reply->error()) {
          QMessageBox::critical(nullptr, QObject::tr("Network Error"),
                                reply->errorString());
...
URLExistsRunnable::URLExistsRunnable(const QString &urlString,
                                     const quint16 port)
    : QRunnable{}, m_url{urlString}, m_port{port} {}
...
void URLExistsRunnable::run() {
  ...
  if (socket.waitForConnected(5000)) {
    socket.write("HEAD " + m_url.path().toUtf8() +
                 " HTTP/1.1\r\n"
                 "Host: " +
                 m_url.host().toUtf8() + "\r\n\r\n");
