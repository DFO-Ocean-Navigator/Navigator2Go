...
void HTMLHighlighter::highlightBlock(const QString &text) {
  ...
  while (startIndex >= 0) {
    ...
    if (subPreviousTag != HTMLHighlighter::Tag) {
      // since the beginning of the tag to the end, if the previous status is
      // not equal Tag
    ...
    while (startQuoteIndex >= 0 &&
      ...
      int quoteLength;
      if (endQuoteIndex == -1) {
        // If a closing quotation mark is found, set the state for the block
        // Quote
        setCurrentBlockState(HTMLHighlighter::Quote);
        quoteLength = text.length() - startQuoteIndex;
      } else {
        quoteLength =
            endQuoteIndex - startQuoteIndex + m_quotes.matchedLength();
      }
  ...
  // EDGES OF TAGS
  // Processing of color tags themselves, that is, highlight words div, p,
  // strong etc.
  ...
  while (startCommentIndex >= 0) {
    ...
    const auto endCommentIndex =
        m_commentEndExpression.indexIn(text, startCommentIndex);
    int commentLength;
    ...
    startCommentIndex = m_commentStartExpression.indexIn(
