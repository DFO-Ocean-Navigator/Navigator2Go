...
/***********************************************************************************/
DialogDatasetView::DialogDatasetView(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::DatasetView) {
  m_ui->setupUi(this);
...
/***********************************************************************************/
DialogDatasetView::~DialogDatasetView() { delete m_ui; }
...
void DialogDatasetView::SetData(const QString &datasetKey,
  ...
  m_ui->checkBoxDatasetEnabled->setCheckState(
      object["enabled"].toBool() ? Qt::Checked : Qt::Unchecked);
  ...
  if (const auto &url{object["url"].toString()}; !url.isEmpty()) {
    ...
    m_ui->lineEditURL->setText(
        QStringLiteral("http://localhost:8080/thredds/dodsC/%1/aggregated.ncml")
            .arg(datasetKey));
  }
  ...
  for (const auto &key : variables.keys()) {
    m_ui->tableWidgetVariables->insertRow(
        m_ui->tableWidgetVariables->rowCount());
    ...
    // Name
    m_ui->tableWidgetVariables->setItem(
        rowIdx, 1, new QTableWidgetItem(variables[key]["name"].toString()));
    // Units
    m_ui->tableWidgetVariables->setItem(
        rowIdx, 2, new QTableWidgetItem(variables[key]["unit"].toString()));
    // Scale
    const auto &scaleArray = variables[key]["scale"].toArray();
    m_ui->tableWidgetVariables->setItem(
        rowIdx, 3,
        new QTableWidgetItem(QString::number(scaleArray[0].toDouble())));
    m_ui->tableWidgetVariables->setItem(
        rowIdx, 4,
        new QTableWidgetItem(QString::number(scaleArray[1].toDouble())));
    ...
    m_ui->tableWidgetVariables->setItem(
        ...
        new QTableWidgetItem(
            QString::number(!factor.isUndefined() ? factor.toDouble() : 1.0)));

    // Hidden
    // Gonna use a checkbox
    auto *const hidden = new QTableWidgetItem();
    hidden->setCheckState(variables[key]["hide"].toBool() ? Qt::Checked
                                                          : Qt::Unchecked);
...
/***********************************************************************************/
void DialogDatasetView::SetData(const QJsonObject &datasetObj,
  ...
  const auto &idx =
      m_ui->comboBoxQuantum->findText(datasetObj["quantum"].toString());
  m_ui->comboBoxQuantum->setCurrentIndex(idx);
  ...
  Network::MakeAPIRequest(
      nam,
      QStringLiteral("http://navigator.oceansdata.ca/api/variables/?dataset=") +
          datasetIDString,
      [&](const auto &doc) {
  ...
  Network::MakeAPIRequest(
      nam,
      QStringLiteral(
          "http://navigator.oceansdata.ca/api/timestamps/?dataset=") +
          datasetIDString,
      [&](const auto &doc) {
        const auto &root = doc.array();
        ...
        const auto &startDate{
            QDate::fromString(start, Qt::DateFormat::ISODate)};
        const auto &endDate{QDate::fromString(end, Qt::DateFormat::ISODate)};

        // Use correct time picking widget
        if (quantum == QStringLiteral("day") ||
            quantum == QStringLiteral("hour")) {
...
std::pair<QString, QJsonObject> DialogDatasetView::GetData() const {
  ...
  obj.insert(QStringLiteral("enabled"),
             m_ui->checkBoxDatasetEnabled->isChecked());
  ...
  obj.insert(QStringLiteral("help"),
             m_ui->plainTextEditHelp->document()->toRawText());
  ...
  for (auto i = 0; i < rowCount; ++i) {
    ...
    const auto scaleMin{
        m_ui->tableWidgetVariables->item(i, 3)->text().toDouble()};
    const auto scaleMax{
        m_ui->tableWidgetVariables->item(i, 4)->text().toDouble()};
    var.insert(QStringLiteral("scale"), QJsonArray({scaleMin, scaleMax}));

    const auto factor{
        m_ui->tableWidgetVariables->item(i, 5)->text().toDouble()};
...
int DialogDatasetView::addEmptyVariable() {
  ...
  // Scale factor
  m_ui->tableWidgetVariables->setItem(rowIdx, 5,
...
/***********************************************************************************/
void DialogDatasetView::checkInputEmpty(const QString &inputLabel,
                                        const QString &inputText) {
  if (inputText.isEmpty()) {
    QMessageBox::critical(this, tr("Alert"),
                          inputLabel + tr(" cannot be empty!"),
                          QMessageBox::Ok);
  }
}
...
void DialogDatasetView::on_pushButtonDeleteVariable_clicked() {
  const auto &reply{QMessageBox::question(this, tr("Confirm Action"),
                                          tr("Delete selected variable?"),
                                          QMessageBox::Yes | QMessageBox::No)};

  if (reply == QMessageBox::Yes) {
    m_ui->tableWidgetVariables->removeRow(
        m_ui->tableWidgetVariables->currentRow());
  }
}
...
void DialogDatasetView::on_pushButtonMagicScan_clicked() {
  ...
  auto *const task{
      new Network::URLExistsRunnable{m_ui->lineEditURL->text(), 8080}};
  ...
  QObject::connect(
      ...
      [&](const auto success) {
        if (success) {
          ...
          const netCDF::NcFile ds{m_ui->lineEditURL->text().toStdString(),
                                  netCDF::NcFile::read};
          if (ds.isNull()) {
            return;
          }
          ...
          if (DialogSelectVars dlg{ds, this}; dlg.exec()) {
            ...
            for (const auto &var : selectedVars) {
              ...
              if (!name_att.isNull()) {
                ...
                m_ui->tableWidgetVariables->item(rowIdx, 1)->setText(
                    QString::fromStdString(long_name));
              }
              // Units
              ...
              if (!units_att.isNull()) {
                ...
                m_ui->tableWidgetVariables->item(rowIdx, 2)->setText(
          ...
          QMessageBox::critical(
              this, tr("Error..."),
              tr("Magic scan failed :( There could be a network issue (did you "
                 "start your THREDDS server?), or the URL is wrong."));
        }
