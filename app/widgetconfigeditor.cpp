...
/***********************************************************************************/
WidgetConfigEditor::WidgetConfigEditor(QWidget *parent,
                                       const Preferences *prefs)
    : QWidget{parent}, m_ui{new Ui::WidgetConfigEditor}, m_prefs{prefs} {
  m_ui->setupUi(this);
  ...
  const auto &offlineConfig{
      m_prefs->ONInstallDir +
      QStringLiteral("/oceannavigator/datasetconfigOFFLINE.json")};
...
/***********************************************************************************/
WidgetConfigEditor::~WidgetConfigEditor() { delete m_ui; }

/***********************************************************************************/
void WidgetConfigEditor::addDatasetToConfigList() {
  bool ok;
  const auto name{QInputDialog::getText(
      this, tr("New dataset key"), tr("Dataset key (e.g. giops_day):"),
      QLineEdit::Normal,
      QStringLiteral("new_dataset_") + QString::number(qrand()), &ok)};
  ...
  if (ok) {
    ...
    on_listWidgetActiveDatasets_itemDoubleClicked(
        m_ui->listWidgetActiveDatasets->item(rowCount - 1));
  }
}
...
void WidgetConfigEditor::on_pushButtonDeleteDataset_clicked() {
  ...
  if (!items.empty()) {
    const auto reply = QMessageBox::question(
        this, tr("Confirm Action"), tr("Delete selected dataset(s)?"),
    ...
    if (reply == QMessageBox::Yes) {
      for (auto *item : items) {
        ...
        delete m_ui->listWidgetActiveDatasets->takeItem(
            m_ui->listWidgetActiveDatasets->row(item));
      }
    }
  }
...
void WidgetConfigEditor::on_pushButtonLoadCustomConfig_clicked() {
  ...
  const auto filePath{QFileDialog::getOpenFileName(
      this, tr("Select a dataset config file..."), m_prefs->ONInstallDir,
...
/***********************************************************************************/
void WidgetConfigEditor::on_listWidgetActiveDatasets_itemDoubleClicked(
...
void WidgetConfigEditor::updateDatasetListWidget() {
  ...
  m_ui->labelActiveConfigFile->setText(
      tr("Active Config File: ") + QFileInfo(m_activeConfigFile).fileName());
...
void WidgetConfigEditor::setDefaultConfigFile() {
  static const auto &offlineConfig{
      m_prefs->ONInstallDir +
      QStringLiteral("/oceannavigator/datasetconfigOFFLINE.json")};
