...
bool addDataset(const QString &rootCatalogFolder, const QString &datasetName,
  ...
  child.append_attribute("xlink:title") = datasetName.toStdString().c_str();
  child.append_attribute("xlink:href") =
      QString(path + ".xml").toStdString().c_str();
  ...
  if (!aggregate.has_value()) {
    ...
    if (dir.entryInfoList().empty()) {
      timeDimension = QInputDialog::getText(
          nullptr, QObject::tr("Enter name of time dimension..."),
          QObject::tr(
              "Please enter the name of the time dimension for your dataset: "
              "it should be something like time, or time_counter."));
      ...
      timeDimension =
          FindTimeDimension(dir.entryInfoList()[0].absoluteFilePath());
    }
    ...
    aggregation.append_attribute("dimName") =
        timeDimension.toStdString().c_str();
  ...
  /* Show aggregation URL dialog...not really needed anymore since it's
  auto-filled in the dataset
   * properties
...
void removeDataset(const QString &rootCatalogFolder, const QString &datasetName,
  ...
  // Modify catalog.xml
  {
    ...
    const auto &nodeToRemove{doc->child("catalog").find_child_by_attribute(
        "catalogRef", "xlink:title", datasetName.toStdString().c_str())};
  ...
  QFile::remove(rootCatalogFolder + QStringLiteral("/catalogs/") + datasetName +
                QStringLiteral(".xml"));
...
/***********************************************************************************/
bool datasetExists(const QString &threddsCatalogLoc,
                   const QString &datasetName) {
  const auto &doc{readXML(threddsCatalogLoc + QStringLiteral("/catalog.xml"))};

  return !doc->child("catalog")
              .find_child_by_attribute("catalogRef", "xlink:title",
...
void createNewCatalogFile(const QString &threddsContentPath,
  ...
  catalog.append_attribute("xmlns") =
  ...
  datasetScan.append_attribute("location") =
      dataset.Location.toStdString().c_str();

  auto serviceName{datasetScan.append_child()};
  serviceName.set_name("serviceName");
  serviceName.text().set("all");

  const auto &filename{threddsContentPath + QStringLiteral("/catalogs/") +
                       dataset.DatasetName.toLower() + ".xml"};
...
pugi::xml_document createNewAggregateFile() {
  ...
  netCDFRoot.append_attribute("xmlns") =
...
void createNewPrimaryCatalog(const QString &threddsContentPath) {
  ...
  catalog.append_attribute("xmlns") =
      "http://www.unidata.ucar.edu/namespaces/thredds/InvCatalog/v1.0";
  catalog.append_attribute("xmlns:xlink") = "http://www.w3.org/1999/xlink";
  catalog.append_attribute("xmlns:xsi") =
      "http://www.w3.org/2001/XMLSchema-instance";
  catalog.append_attribute("xsi:schemaLocation") =
      "http://www.unidata.ucar.edu/namespaces/thredds/InvCatalog/v1.0 "
