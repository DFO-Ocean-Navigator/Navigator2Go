...
const QHash<int, QString> indexToMonth{
    {1, QStringLiteral("January")},   {2, QStringLiteral("February")},
    {3, QStringLiteral("March")},     {4, QStringLiteral("April")},
    {5, QStringLiteral("May")},       {6, QStringLiteral("June")},
    {7, QStringLiteral("July")},      {8, QStringLiteral("August")},
    {9, QStringLiteral("September")}, {10, QStringLiteral("October")},
    {11, QStringLiteral("November")}, {12, QStringLiteral("December")}};

const QHash<QString, int> monthToIndex{
    {QStringLiteral("January"), 1},   {QStringLiteral("February"), 2},
    {QStringLiteral("March"), 3},     {QStringLiteral("April"), 4},
    {QStringLiteral("May"), 5},       {QStringLiteral("June"), 6},
    {QStringLiteral("July"), 7},      {QStringLiteral("August"), 8},
    {QStringLiteral("September"), 9}, {QStringLiteral("October"), 10},
    {QStringLiteral("November"), 11}, {QStringLiteral("December"), 12}};

/***********************************************************************************/
WidgetMonthPicker::WidgetMonthPicker(QWidget *parent)
    : QWidget{parent}, m_ui{new Ui::WidgetMonthPicker} {
  m_ui->setupUi(this);
}

/***********************************************************************************/
WidgetMonthPicker::~WidgetMonthPicker() { delete m_ui; }
...
std::pair<QDate, QDate> WidgetMonthPicker::getStartEndDate() const {
  return {{m_ui->comboBoxStartYear->currentText().toInt(),
           monthToIndex[m_ui->comboBoxStartMonth->currentText()], 1},
          {m_ui->comboBoxEndYear->currentText().toInt(),
           monthToIndex[m_ui->comboBoxEndMonth->currentText()], 1}
...
/***********************************************************************************/
void WidgetMonthPicker::on_comboBoxStartYear_currentIndexChanged(
    const QString &arg1) {
  populateMonth(arg1.toInt(), m_ui->comboBoxStartMonth);
}

/***********************************************************************************/
void WidgetMonthPicker::on_comboBoxEndYear_currentIndexChanged(
    const QString &arg1) {
...
void WidgetMonthPicker::validateMonthSelection() {
  ...
  if (m_ui->comboBoxStartYear->currentText() !=
      m_ui->comboBoxEndYear->currentText()) {
    return;
  }

  // If the start month is after the end month
  if (m_ui->comboBoxStartMonth->currentIndex() >
      m_ui->comboBoxEndMonth->currentIndex()) {
